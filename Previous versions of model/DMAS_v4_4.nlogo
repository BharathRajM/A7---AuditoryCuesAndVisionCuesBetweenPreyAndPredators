...
[
  ...
  original_predator-vision
  original_vision
  original_audio_range
  original_audioduration
]
...
to setup ;; clears and initializes the experiment setup
  clear-all
  ;back up values for changing back after decreasing value in green patches
  set original_predator-vision predator-vision
  set original_vision vision
  set original_audio_range audio_range
  set original_audioduration audioduration
...
;;to avoid-trees-prey
  ;; let front-patches patches in-cone 2 75
   ;ask front-patches [
    ;if pcolor = green [rt (prey-turn-coefficient / 100) * -180]
   ;]
;end

;to avoid-trees-predator
 ;  let front-patches patches in-cone 2 75
  ; ask front-patches [
  ;  if pcolor = green [rt (predator-turn-coefficient / 100) * -180]
  ; ]
;end

to check-patch
  ;;identify if it is in the auditory patch
  ;set vision original_vision
  ;set audio_range original_audio_range
  ;set audioduration original_audioduration
  if pcolor = green [
    set speed (0.5 * min-prey-speed)
    ;set vision (0.5 * vision)
    ;set audio_range (0.5 * audio_range)
    ;set audioduration (0.5 * audioduration)
  ]
   if pcolor = yellow
   [
...
to check-patch-predator
  ;set predator-vision original_predator-vision
  if pcolor = green [
    set speed (0.5 * min-predator-speed)
    ;set predator-vision (0.5 * predator-vision)
  ]
end
...
to scare-right  ;; prey on right side will flee right
  ...
  ifelse pcolor = green
    [set vision_value (vision * 0.5)
     ...
     set duration_value (audioduration * 0.5)
    ]
