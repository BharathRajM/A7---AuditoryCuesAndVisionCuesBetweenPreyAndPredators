...
to setup ;; clears and initializes the experiment setup
  ...
  set flock-color-list ( list grey orange turquoise cyan sky violet magenta pink )
...
to check-patch-predator
  ;check if predator is in the woods
  if pcolor = black [
...
to scare-right  ;; prey on right side will flee right
  ;;;Based on whether the patch the prey is in is either green or not, the variables for vision, audio_range and audioduration need to be halved.
  let vision_value 0
  let audio_value 0
...
<experiments>
  ...
  <experiment name="main-experiment" repetitions="10" sequentialRunOrder="false" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
